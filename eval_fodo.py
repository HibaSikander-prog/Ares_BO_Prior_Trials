...
MODIFIED VERSION: Now saves learned drift_length values to CSV (like ARES saves misalignments)
...
def main(args):
    ...
    for i in range(args.n_trials):
        ...
        # ========== NEW CODE: Save learned drift_length ==========
        if args.optimizer == "BO_prior" and args.task in ["mismatched", "matched_prior_newtask"]:
            try:
                # Extract the GP model and its prior mean
                model = xopt.generator.model
                gp_model = model.models[0]
                learned_mean = gp_model.mean_module._model
                
                # Get the learned drift_length value
                learned_drift = float(learned_mean.drift_length.item())
                
                # Save it as a column (same value for all rows in this trial)
                xopt.data["learned_drift_length"] = learned_drift
                
                print(f"  Final learned drift_length: {learned_drift:.6f} m")
                
                # Compare to ground truth for mismatched task
                if args.task == "mismatched":
                    true_drift = 0.7
                    error = abs(learned_drift - true_drift)
                    print(f"  Ground truth drift_length: {true_drift:.6f} m")
                    print(f"  Learning error: {error:.6f} m ({error/true_drift*100:.2f}%)")
                elif args.task == "matched_prior_newtask":
                    # For this task, drift was set directly on segment, not via property
                    print(f"  (Initial drift_length was set to 0.7 m via segment)")
                    
            except Exception as e:
                print(f"  Warning: Could not save learned drift_length: {e}")
                import traceback
                traceback.print_exc()
        # ========== END NEW CODE ==========
    ...
    if args.optimizer == "BO_prior" and "learned_drift_length" in df.columns:
        ...
        if args.task == "mismatched":
            print(f"  Ground truth: 0.700000 m")
            print(f"  Mean error: {abs(final_drifts.mean() - 0.7):.6f} m")
    # ========== END NEW CODE ==========

...
that is widely used because of its simplicity and robustness~\cite{nelder1965simplex}. However, NM is best suited for convex optimisation landscapes and tends to struggle in higher-dimensional search spaces. Another approach is the Reinforcement Learning (RL) 
...
making it possible to predict beam properties quickly and accurate enough to be used directly within an optimisation loop. The standard BO uses uninformed zero mean prior function in the GP model. This implies that an algorithm must learn the whole function landscape from scratch and have to begin with no prior knowledge of the intended objective 
...
but they do not compute the gradients directly and must rely on numerical gradient estimation methods such as finite differences, this makes them unsuitable for 

...
European X-ray Free Electron Laser (XFEL). These facilities accelerate charged particles, that enable researchers to study atomic level processes and structures with 
...
parameters using their knowledge and experience, which is not really an efficient method. This method significantly relies 
...
advantages and disadvantages. Nelder-Mead Simplex (NM) is a gradient-free optimization technique and one of the approaches 
that is widely used because of its simplicity and robustness~\cite{nelder1965simplex}. However, NM is best suited for convex optimization landscapes and tends to struggle in higher-dimensional search spaces. Another approach is the Reinforcement Learning (RL) 
...
a desirable option for accelerator tuning since it excels at solving expensive black-box optimization problems, thus BO is already used well with great 
...
which means we can build reliable models based on physics knowledge. Thus, all thanks to modern simulation library like Cheetah~\cite{kaiser2024bridging} that takes advantage of this,
making it possible to predict beam properties quickly and accurate enough to be used directly within an optimization loop. The standard BO uses uninformed zero mean prior function in the Gaussian process model. This implies that 
...
Although, other simulators like Methodical Accelerator Design (MAD-X)~\cite{madx2023} and ELEctron Generation ANd Tracking (elegant)~\cite{elegant2000} have been around for years and are capable of simulating beam dynamics, 
they do not compute the gradients directly and must rely on numerical gradient estimation methods such as finite differences, this makes them unsuitable for 
integration as a GP mean function, which would be highly inefficient as the surrogate model parameters must be updated at every optimization step. Cheetah being built on PyTorch, can compute gradients
directly through automatic differentiation., enabling it to be embedded as the GP mean function with its parameters optimised joinly alongside the
surrogate model.
...
Cheetah has already been demonstrated as a physics-informed prior for BO on a simple and straightforward beam focusing FODO 
(Focus-Drift-Defocus-Drift) lattice~\cite{kaiser2024bridging}. A FODO cell is a fundamental building block of an accelerator lattice design, which is made up of alternating quadrupole magnets 
...
\begin{enumerate}
    ...
    \item Implementing physics-informed prior mean function for the ARES lattice being compatible with the Xopt Bayesian optimization framework

...
and vertical beam sizes, considering only beam size (focusing) without beam position (centering), unlike ARES objective in equation \ref{mae_ares}, 
...
\begin{enumerate}
    \item \textbf{Higher Dimensionality:} In contrast to two dimensional FODO problem, ARES problem possess five dimensional search space including two steering/corrector magnet angles ($\theta_{CV}$ and $\theta_{CH}$) and three quadrupole strengths ($k^{Q1},k^{Q2}, k^{Q3}$). The volume of the search space is greatly increased by around 2.5x in dimensionality, which makes it more difficult and raises the importance of informed prior. 
    \item \textbf{Realistic Lattice:} The ARES Experimental Area ($ARESlatticeStage3v1\_9.json$) is a real accelerator section loaded from an actual lattice description file unlike the synthetic FODO cell. 

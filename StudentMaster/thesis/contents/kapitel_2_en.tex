...
present Bayesian optimization as a framework for sample efficient black-box optimization and then look over the Cheetah simulator and the 
ARES accelerator where the work is used and how physics knowledge may be integrated through the prior mean function.
...
infinite differentiability,  Mat\'ern-5/2 kernel is widely used in Bayesian Optimization~\cite{roussel2024bo}. The GP uses the Bayes’s rule,
...
Bayesian Optimization (BO) is a sequential method for determining the optimal value of expensive black-box functions~\cite{roussel2024bo}. It still works well when the gradient information 
...
\node[block, below=of gp] (acq) {\parbox{5.5cm}{\centering\textbf{Optimize Acquisition} \\ $x_{t+1} = \arg\max \alpha$}};
...
\end{tikzpicture}
\caption{The Bayesian optimization loop.}
...
\begin{itemize}
    ...
    \item \textbf{Upper Confidence Bound:} This acquisition function takes more direct approach by simply combinig the prredicted model and uncertainity.  UCB can be mathematically defined as
            \begin{equation}
                \label{eq_UCB}
...
with $f_{physics}$ being the physics model prediction, $x$ being input to the optimization, which in our case are the five magnet settings required to be optimized ($k_{Q1}$, $k_{Q2}$, $\theta_{CV}$,$k_{Q3}$, $\theta_{CH}$) 
...
every time the GP model is fitted, it optimizes the kernel hyperparameters $\omega$ (length scales, signal variance,
noise variance) by maximising the log marginal likelihood of observed data~\cite{williams2006gaussian}. A key innovation in this project work is making the physics model parameters $\phi$ (in our case quadrupole misalignments: $\Delta_{xQ1}$, $\Delta_{yQ1}$, $\Delta_{xQ2}$, $\Delta_{yQ2}$, $\Delta_{xQ3}$, $\Delta_{yQ3}$) trainable alongside the standard GP hyperparameters, represented as 
...
By optimizing $\log p(y | X, \phi, \omega)$, it measures how well the model explains the observed data. 
...
Additionally, Cheetah enables BO packages like $BoTorch$ to optimize acquisition functions effectively. It provides two beam representations. 
...
upto a 72 $m^{-2}$ field strength. The steering/corrector magnets have a limit of 6.2 mrad. The camera's field of view is approximately 8 mm by 5 mm, with a pixel resolution of 2448 by 2040. The scintillating screen's effective resolution is about 20 $\mu$m~\cite{kaiser2024rl}. 
The beam tuning task involves optimizing the five magnets defined as,
\begin{equation}
    \label{eq_magnets}
...
focusing of the beam into a single metric to minimise. Where $\sigma_x$ and $\sigma_y$ are strictly positive since they are beam size. In a real world accelerator, the quadrupole magnets are never exactly aligned; 

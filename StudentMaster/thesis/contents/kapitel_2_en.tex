...
Two widely used aquisitions funtions are Expected Improvement (EI) and Upper Confidence Bound (UCB).

\begin{itemize}
    \item  \textbf{Expected Improvement:} This acquisition function computes how much better do we expect a candidate point to be compared to our current best observation $f(x^*)$, the expectation is taken over the GP's predictive distribution. EI naturally trades off exploration and exploitation wihtout any tuning parameter. 
            \begin{equation}
    ...
    \item \textbf{Upper Confidence Bound:} This acquisition function takes more direct approach by simply combinig the prredicted model and uncertainity.
            \begin{equation}
                \label{eq_UCB}
...
following the approach used in the first demo over simple FODO problem \cite{kaiser2024bridging} and common practice in accelator 
...
hyperparameter optimization). Cheetah \cite{kaiser2024bridging}, was developed to satisfy these needs. Cheetah makes a conscious trade-off, in order to achieve speed, it gives up some fidelity. 
...
but cheetah overcomes this by utilizing $PyTorch$ automatic differentiation. So, instead of manually deriving gradients or approximating them numerically, which would then require multiple function calls per parameter, $PyTorch$ tracks operations as they happen and can compute exact gradients autometically through backpropagation.
Aditionally, this enables BO packages like $BoTorch$ to optimize acquisition functions effectively.
Cheetah provides two beam representations. 
...
upto a 72 m field strength. The steering/corrector magnets have a limit of 6.2 mrad. The camera's field of view is approximately 8 mm by 5 mm, with a pixel resolution of 2448 by 2040. The scintillating screen's effective resolution is about 20 $\mu$m \cite{kaiser2024rl}. 
...
focusing of the beam into a single metric to minimise. Where $\sigma_x$ and $\sigma_y$ are strictly positive since they are beam size.
\begin{equation}
    \label{eq_ares_ob}

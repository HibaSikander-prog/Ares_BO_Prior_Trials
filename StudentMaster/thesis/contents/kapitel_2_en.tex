...
In this chapter, the theoretical foundations of this project work are developed. We begin with Gaussian processes as probabilistic function approximators, 
...
that learn a predefined set of weights. A Gaussian process is fully specified by its mean function $m(x)$ and covariance function (kernel) $k(x,x')$.
\begin{equation}
\label{gp_formula}
f(x) \sim \mathcal{GP}(m(x), k(x,x'))
\end{equation}

Here equation \ref{gp_formula} states that a function $f(x)$ is drawn from GP where the mean function $m(x)$ encodes previous expectations regarding the behavior of the 
...
infinite differentiability,  Mat\'ern-5/2 kernel is widely used in Bayesian Optimization \cite{roussel2024bo}. The GP uses Bayes’s rule (equation \ref{eq_bayes_rule}) to update 
...
method \cite{jalas2021bolaserplasma} \cite{Duris2020bofreeelectron}. The goal is to solve, shown in equation \ref{eq_optFormulation}:
...
As shown in Figure~\ref{fig:bayesian_optimization_loop}, the optimization process is iterative, We start with a modest initial set of observations, fit a GP model to the data and then choose the next point to evaluate using an acquisition function.
...
following the approach in BO with cheetah prior's first demo \cite{kaiser2024bridging} and common practice in accelator 
optimization \cite{roussel2024bo}. 
\begin{equation}
    \label{eq_UCB}
    \alpha_{UCB}(x) = \mu(x) + \beta \sigma(x)
\end{equation}

Where $\mu(x)$ and $\sigma(x)$ are the GP posterior mean and standard deviation respectively and parameter $\beta$ controls the exploration-exploitation trade-off. 
...
learned from fewer observations when the prior mean is accurate. Whereas when the prior is simply zero, the GP has to learn the entire function from scratch. This motivates using a physics informed prior mean function (equation \ref{eq_physics_prior}):
...
Where $f_{physics}$ is the physics model prediction, $x$ being input to the optimization, which in our case are the five magnet settings required to be optimized ($k_{Q1}$, $k_{Q2}$, $\theta_{CV}$,$k_{Q3}$, $\theta_{CH}$) 
...
In standard GP practice, as shown in equation \ref{eq_bo_kernel_hyperparameters}, every time the GP model is fitted, it optimizes the kernel hyperparameters $\omega$ (length scales, signal variance,
noise variance) by maximising the log marginal likelihood of observed data \cite{williams2006gaussian}. 
...
A key innovation in this project work is making the physics model parameters $\phi$ (in our case quadrupole misalignments: $\Delta_{xQ1}$, $\Delta_{yQ1}$, $\Delta_{xQ2}$, $\Delta_{yQ2}$, $\Delta_{xQ3}$, $\Delta_{yQ3}$) trainable alongside the standard GP hyperparameters, represented by equation \ref{eq_bo_physics_parameters}. 
As more data is collected the learned $\phi$ gets closer to the true values so the physics model becomes more accurate eventually leading to better prediction and optimization performance. 
...
Thus here, y are the observed objective values, X are input points evaluated so far and $\phi$ are the physics model parameters. By optimizing $\log p(y | X, \phi, \omega)$, it measures how well the model explains the observed data. 
Note that $\omega$ is always optimized during standard GP fitting, while $\phi$ is only optimized when explicitely set as trainable. This approach allows the physics model to adapt better and gradually converge towards their true values, making the prior more accurate and speeding up the convergence by mitigating the mismatch between the model and reality.
...
Using physics knowledge based simulator as a prior mean function requires both speed (so the simulator must be fast enough to call within each GP evaluation) and differentiability (in order to facilitate gradient based 
hyperparameter optimization). Cheetah \cite{kaiser2024bridging}, was developed to satisfy these needs, its main objective was to give machine learning applications a differentiable beam dynamics code with improved speed over existing simulation codes.
This ability of differentiation in Cheetah is important for our approach because when GP fits its hyperparameter by maximising marginal likelihood, gradients flow through the Cheetah simulation to update the trainable misalignment parameter. Moreover, cheetah makes a conscious trade-off, in order to achieve speed, it gives up some fidelity. 

Gradients of beam dynamics are often expensive to compute analytically or numerically \cite{kaiser2024bridging} 
but cheetah overcomes this by utilizing $PyTorch$ automatic differentiation. Aditionally, this enables BO packages like $BoTorch$ to optimize acquisition functions effectively.
Cheetah elements, by default, compute linear beam dynamics through an application of the linear transfer map $R_{Cheetah}$$\in$$\mathbb{R}^{7 \times 7}$, which is already provided with $R_{0}$$\in$$\mathbb{R}^{6 \times 6}$  being the standard transfer matrix refrenced in \cite{Brown_1968hkv}. 
\begin{equation}
R_{\text{Cheetah}} = \begin{pmatrix}
R_0 & \vdots \\
0\cdots 0 & 1
\end{pmatrix}
\label{eq:cheetah}
...
The additional dimension allows the representation of effects like quadrupole misalignments, which is directly relevant to this project work since we learn these trainable parameters $\phi$.
Cheetah provides 
...
\begin{itemize}
    ...
    \item \textbf{Particle Beam:} represents the beam as individual macro-particles, each decribed by seven dimenional vector. This allows modeling of non-Gaussian distribution and bunch substructures, but at greater computational cost. 
\end{itemize}


This project work uses parameter beam because the ARES experimental area runs in the linear regime, where the Gaussian assumptions hold and thus the speed advantage is crucial, since cheetah simulator is being called wihtin every GP evaluation. 
...
techniques on them. It is an S-band radio frequency linac with two independently powered travelling wav accelerating strcutures and a photoinjector. These structures can operate at energies upto 154 MeV. 
ARES's main goal is to create and investigate sub-femtosecond electron bunches at accelerated energy. For uses like radiation production by free leectron lasers, the capacity to produce such brief bunches is highly desirable. ARES is also used for medical applications and for the study and development of accelerator components \cite{kaiser2024rl}.    \

The project work focuses on the ARES experimental area. The beamline consists of three quadrupole magnets (Q1, Q2, Q3) and 
...
and finally the horizontal corrector magnet (CH) before reaching the screen. Each magnet is separated by a drift space ($d_1$, $d_2$, $d_3$, $d_4$, $d_5$, $d_6$) each having lengths within the range of 0.175 to 0.450 meters.
...
upto a 72 m field strength. The steering/corrector magnets have a limit of 6.2 mrad. The camera's field of view is approximately 8 mm by 5 mm, with a pixel resolution of 2448 by 2040. The scintillating screen's effective resolution is about 20 $\mu$m. 
...
Where in equation \ref{eq_magnets},  $\mathbf{u} \in \mathbb{R}^5$. In order to produce a focused and centred beam at screen, The measured observables are beam’s 
horizontal and vertical positions ($\mu_x$ and $\mu_y$) and sizes ($\sigma_x$ and $\sigma_y$). The objective 
function (equation \ref{eq_ares_ob}) used throughout this work is the mean absolute error which combines both centering and 
focusing of the beam into a single metric to minimise.
\begin{equation}
    \label{eq_ares_ob}
    MAE = \frac{1}{4}(|\mu_x| + |\sigma_x| + |\mu_y| + |\sigma_y|)
\end{equation}

In a real world accelerator, the quadrupole magnets are never exactly aligned; 
...
diverting the beam. These misalignments are not directly measurable during operation. By treating the six misalignments parameters ($\Delta_{xQ1}$, $\Delta_{yQ1}$, $\Delta_{xQ2}$, $\Delta_{yQ2}$, $\Delta_{xQ3}$, $\Delta_{yQ3}$) 
...
and allows the model to adjust to actual accelerator settings using the optimization data. 

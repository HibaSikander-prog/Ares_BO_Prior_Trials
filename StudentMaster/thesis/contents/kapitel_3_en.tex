...
\begin{itemize}
    ...
    \item \textbf{\texttt{eval\_ares.py}} constructs the optimisers (Standard BO, BO with Cheetah prior and Nelder-Mead) along with the experimental scenarios (matched, mismatched and matched\_prior\_newtask) and loops through the optimisation trials. 
    \item \textbf{\texttt{eval\_ares\_metrics.py}} it computes the performance metrics and generates a final csv with all the metrics to compare the techniques. 
...
10 separate trials are conducted per configuration and the results are saved to CSV file for further analysis. 
...
The mean, median and standard deviation are computed for each metrics across the 10 trials.

...
In this project, Cheetah's \texttt{ParameterBeam} is used to describe the beam entering the ARES Experimental Area. \texttt{ParameterBeam} was used 
...
$\pm72 m^{-2}$ to our simulation's $\pm30 m^{-2}$, and from $\pm 6.2 mrad$ to $\pm 6.0 mrad$ respectively because the optimization consistently converges well within the narrower range
...
centred on the beam. To let the optimizer know how good or bad the result is, we need a single number. Following the previous FODO demonstration \cite{kaiser2024bridging}, for ARES Experimental Area, we use the mean absolute error as objective function, including each of the four beams observable, defined as
...
with $\mu_x$ and $\mu_y$ being the beam positions (ideally, zero means beam is centred) and $\sigma_x$ and $\sigma_y$ being the beam sizes (the smaller the better), simultaneously centering and focusing the beam. 
...
\begin{table} [htbp]
    ...
    \begin{tabular}{p{4.0cm} p{4.0cm} p{7.0cm}}
        ...
        \midrule
        Raw Parameter & \texttt{register\_parameter()} & A raw unconstrained \texttt{nn.Parameter} is created at first, which is what PyTorch’s optimizer updates directly.\\
...
\subsection{Optimization Runner}
...
Three optimizers are compared Listing \ref{code:mismatched_setup} shows how the most complex one (BO with mismatched prior) is set up.
the other two are the simpler subsets of the same pattern. 
...
prior_mean_module = bo_cheetah_prior.AresPriorMean(
                ...
                # Initialize misaligments to zero, will be learned
...
optimizer more information about where the magnets are actually positioned and the prior gradually gets more accurate 
...
\begin{enumerate}
    \item \textbf{Best MAE:} is the best (minimum) MAE reached across all steps within each trial, defined as 
    \begin{equation}
    ...
    \item \textbf{RMSE:} is the root mean square across all iterations within each trial, capturing how quickly the optimizer improves and not just the final value, defined as
    \begin{equation}
        \text{RMSE}^{(i)} = \sqrt{\frac{1}{4(T+1)} \sum_{t=0}^{T} \left( (\mu_x^{(i,t)})^2 + (\mu_y^{(i,t)})^2 + (\sigma_x^{(i,t)})^2 + (\sigma_y^{(i,t)})^2 \right)}.
    \end{equation}
    \item \textbf{Improvement Best percentage(\%):} is the improvement measured from initial to best mae, computed as $((\text{MAE}_0 - \text{MAE}_{\text{best}})/\text{MAE}_0) \times 100$.
    \item \textbf{Improvement Final percentage(\%):} is the improvement measured from initial to final mae, computed as $(((\text{MAE}_0 - \text{MAE}_{\text{final}})/\text{MAE}_0) \times 100)$.

    \item \textbf{Steps to target:} shows how many evaluations does it take to drop below the threshold of 40 $\mu$m of MAE for the first time.
...
and the steps to target are also informative because it tells how quickly the optimizer gets the beam into an acceptable state. \texttt{plot\_ares\_results.py} 
produces plots and tables, styled to match in \cite{kaiser2024bridging} and \cite{kaiser2024rl}, which can then be seen later under Section 4.  